...
const AddStudent = () => {
  ...
  const createStudent = async () => {
    ...
    setLoading(true);
    try {
      await studentService.createStudent(inputValues);
      ...
      <CRow>
        <CCol xs="12" lg="12">
          <CCard>
            ...
            <CCardFooter>
              {
                loading ? <CButton className="float-right" color="primary" disabled={loading}>...Loading</CButton> : <CButton className="float-right" type="submit" color="primary" onClick={createStudent}><CIcon name="cil-scrubber" /> Thêm học sinh</CButton>
              }
            </CCardFooter>
          </CCard>
        </CCol>
